...
```
```{r}
# Drop NAs in Surface and create data matrices
...
# Split into matrices per surface type
surface_matrices <- lapply(surface_levels, function(s) {
  surface_data %>%
    filter(Surface == s) %>%
    select(spread_favorite_scaled, over_under_line_scaled) %>%
    drop_na() %>%
...
# Run MANOVA for Surface
Y_surface <- do.call(rbind, surface_matrices)
group_surface <- factor(rep(surface_levels, times = sapply(surface_matrices, nrow)))

fit_surface <- manova(Y_surface ~ group_surface)
surface_results <- c(
  Wilks = summary(fit_surface, test = "Wilks")$stats[1, "Pr(>F)"],
  Pillai = summary(fit_surface, test = "Pillai")$stats[1, "Pr(>F)"],
  Hotelling = summary(fit_surface, test = "Hotelling-Lawley")$stats[1, "Pr(>F)"],
  Roy = summary(fit_surface, test = "Roy")$stats[1, "Pr(>F)"]
)
